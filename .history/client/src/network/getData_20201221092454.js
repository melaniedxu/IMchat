...
export const twilioMediaAuth = async () => {
  // const res = await axios.get(`https://ancient-river-99811.herokuapp.com/https://mcs.us1.twilio.com/v1/Services`, {
  //   auth: {
  //     username: 'AC9eb25c66af41aae40b9be69660922a5d',
  //     password: 'fc6e0c4916cf4f33d087fc67cde3df43',
  //   },
  // });
  const res = await axios.get(`${herokuBaseURL}messages/auth`);
  console.log(res);
  return res;
}; 

export const twilioMediaUpload = async (curFile, imageType) => {
  // const res = await axios({
  //   method: 'post',
  //   url: `https://ancient-river-99811.herokuapp.com/https://mcs.us1.twilio.com/v1/Services/IS960c89f737814fd7baa53c4cf10a34b8/Media`,
  //   data: binStr,
  ...
  console.log('mediafile', curFile);
  formdata.append('file', curFile);
  for (var pair of formdata.entries()) {
    console.log('media data', pair[0]+ ', ' + pair[1]); 
  }
  // const res = await axios.post(`${herokuBaseURL}messages/media`, formdata);
  const res = await axios({
    method: 'post',
    url: `${herokuBaseURL}messages/media`,
    data: formdata,
  ...
  if (res.status !== 200) {
    console.log('[twilioMediaUpload error]', res.error);
    return null;
  }
...
export const getMessagesByConversation = async (conversationId, userId, readUpdate = false) => {
  ...
  const finalObjArr = await Promise.all((messages).map(async (msg) => {
    ...
    const msgMediaURL = msg.media[0].sid;
    console.log('msgMediaURL', msgMediaURL);
    const mediaResp = await axios.get(`${herokuBaseURL}messages/mediaURL`, {
      params: {
        msgMediaURL,
      },
    });
    console.log('mediaURL resp', mediaResp);
