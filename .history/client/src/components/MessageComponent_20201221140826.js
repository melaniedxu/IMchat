...
import DeleteMediaMessageButton from './DeleteMediaMessageButton';
...
function MessageChat({ user, contactList }) {
  ...
  // (function() {
  //   var cors_api_host = 'ancient-river-99811.herokuapp.com';
  ...
  useEffect(() => {
    ...
    const tmpObj = messageList.map((msg) => {
      if (msg.message_type === 'text') { // text
        ...
        const deletebutton = (
          <div id="converDiv">
            {console.log('message component userid and contact id', userId, contact)}
            <DeleteMessageButtonComponent
              userid={userId}
              contactid={contact}
        ...
        const imageComponent = (
          <>
            <DeleteMediaMessageButton conversationId={conversationId} MessageId={messageId} />
            <a
              target="_blank"
              href={msg.mediaURL}
            >
  ...
  const uploadMedia = async (e) => {
    ...
    if (!validateFile(curFile)) {
      ...
      try {
        ...
        // data.append('upload', curFile);
        const mediaSID = await twilioMediaUpload(curFile, curFile.type);
        // const mediaSID_resp = await axios({
        //   method: 'post',
        ...
        //   }
        // });
        // const mediaSID = mediaSID.data.sid;
